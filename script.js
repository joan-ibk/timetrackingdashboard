// get all acees to tabs and panels
...
function handleKeyDownEvent(e) {
    ...
    if (!actEl.classList.contains('tabs')) { return }
    switch (e.key) {
        case 'ArrowLeft':
            e.preventDefault();
            if (actEl === tabs[0]) {
...
function generateCardHTMLString(data) {
    const htmlString = Object.entries(data).map(time => {
        ...
        return `
        <div class="container--timecards" role="tabpanel" aria-labelledby="tab-${timePeriod}" id="panel-${timePeriod}" aria-hidden="${timePeriod === "daily" ? "false" : "true"
            }">
